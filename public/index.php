<!DOCTYPE html><html lang="en" data-bs-theme="light" data-pwa="true">
  
    <!-- HEAD ...........................-->
    <?php include("./includes/head.php") ?>
    <!-- HEAD ...........................-->

  <!-- Body -->
  <body> 
  
    <!-- HEADER ...........................-->
    <?php include("./includes/header.php") ?>
    <!-- HEADER ...........................-->


    <!-- Page content -->
    <main class="content-wrapper">

      <!-- // DEBUT SECTION BLOCK 1 D'ACCEUILLE -->
      <section class="position-relative bg-info overflow-hidden py-5">
        <!-- Éléments décoratifs subtils -->
        <div class="position-absolute top-0 start-0 w-100 h-100">
            <!-- Motif de fond discret -->
            <div class="position-absolute top-50 start-50 translate-middle opacity-10" style="width: 100%; max-width: 600px;">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 400" class="text-white">
                    <path d="M100,100 Q200,50 300,100 T500,100" fill="none" stroke="currentColor" stroke-width="2"/>
                    <path d="M100,200 Q250,150 400,200 T700,200" fill="none" stroke="currentColor" stroke-width="2"/>
                    <circle cx="150" cy="150" r="8" fill="currentColor"/>
                    <circle cx="350" cy="150" r="8" fill="currentColor"/>
                    <circle cx="550" cy="150" r="8" fill="currentColor"/>
                    <circle cx="250" cy="250" r="8" fill="currentColor"/>
                    <circle cx="450" cy="250" r="8" fill="currentColor"/>
                    <circle cx="650" cy="250" r="8" fill="currentColor"/>
                </svg>
            </div>
        </div>

        <div class="container position-relative z-3 py-5 py-sm-6 py-lg-7">
            <!-- Contenu centré -->
            <div class="text-center mx-auto" style="max-width: 850px;">
                <!-- Titre principal -->
                <h1 class="display-3 text-white fw-bold mb-4">
                    Votre expert en matériaux <span class="text-warning">&</span> outillage
                </h1>
                
                <!-- Sous-titre -->
                <p class="lead text-light mb-5 px-3">
                    Tout ce dont vous avez besoin pour vos projets, du bricolage aux chantiers professionnels. 
                    Un large choix, des prix compétitifs et un service personnalisé.
                </p>
                
                <!-- Séparateur décoratif -->
                <div class="mx-auto mb-5" style="width: 100px; height: 3px; background: rgba(255, 255, 255, 0.3);"></div>
                
                <!-- Points clés -->
                <div class="row justify-content-center mb-5">
                    <div class="col-md-4 mb-4 mb-md-0">
                        <div class="text-center px-3">
                            <div class="bg-white bg-opacity-10 rounded-circle d-inline-flex align-items-center justify-content-center mb-3" 
                                style="width: 70px; height: 70px;">
                                <i class="bi bi-shield-check text-white fs-3"></i>
                            </div>
                            <h4 class="text-white h5 fw-bold">Qualité garantie</h4>
                            <p class="text-light small mb-0">Matériaux certifiés et outils de marques reconnues</p>
                        </div>
                    </div>
                    
                    <div class="col-md-4 mb-4 mb-md-0">
                        <div class="text-center px-3">
                            <div class="bg-white bg-opacity-10 rounded-circle d-inline-flex align-items-center justify-content-center mb-3" 
                                style="width: 70px; height: 70px;">
                                <i class="bi bi-truck text-white fs-3"></i>
                            </div>
                            <h4 class="text-white h5 fw-bold">Livraison rapide</h4>
                            <p class="text-light small mb-0">Disponible en magasin ou livré sous 24h-48h</p>
                        </div>
                    </div>
                    
                    <div class="col-md-4">
                        <div class="text-center px-3">
                            <div class="bg-white bg-opacity-10 rounded-circle d-inline-flex align-items-center justify-content-center mb-3" 
                                style="width: 70px; height: 70px;">
                                <i class="bi bi-headset text-white fs-3"></i>
                            </div>
                            <h4 class="text-white h5 fw-bold">Conseils experts</h4>
                            <p class="text-light small mb-0">Notre équipe vous guide dans vos choix techniques</p>
                        </div>
                    </div>
                </div>
                
                <!-- Boutons d'action -->
                <div class="d-flex flex-wrap justify-content-center gap-3 mb-5">
                  <a href="#produits" class="btn btn-light btn-lg px-5 py-3 fw-bold shadow-sm">
                      <i class="bi bi-tools me-2"></i>
                      Voir les produits
                  </a> 
                </div> 
                
                <!-- Appel à action secondaire -->
                <div class="mt-4">
                    <p class="text-light small mb-0">
                        <i class="bi bi-info-circle me-1"></i>
                        Besoin d'un produit spécifique ? 
                        <a href="#contact" class="text-warning text-decoration-none fw-medium">Contactez-nous</a> 
                        pour une recherche personnalisée.
                    </p>
                </div>
            </div>
        </div>
      </section>
      <!-- // FIN SECTION BLOCK 1 D'ACCEUILLE -->

      <!-- LISTE DES CATEGORIES DISPONIBLES -->
      <section class="container pt-2 pt-sm-3 pt-md-4 pt-lg-5 pb-5 mb-xxl-3" id="categories">
        <div class="d-flex align-items-start justify-content-between gap-4 pt-5 pb-3 mb-2 mb-sm-3 mb-lg-4">
          <h2 class="mb-0">Nos catégories de produits</h2> 
        </div>
        <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-3 g-sm-4">

          <!-- Catégorie -->
          <div class="col">
            <article class="hover-effect-scale position-relative d-inline-flex align-items-center gap-3">
              <div class="position-relative d-flex align-items-center justify-content-center text-secondary-emphasis" style="width: 56px; height: 56px">
                <svg class="position-relative z-1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"><path d="M24 5.516a3.27 3.27 0 0 0-3.265-3.266 3.27 3.27 0 0 0-3.265 3.266v1.84H.953c-.526 0-.953.428-.953.953l.063 12.84c.119.369.514.617.89.617h19.781A3.27 3.27 0 0 0 24 18.5V5.516zm-6.531 8.9l-3.601 1.927c-.529.283-.099 1.134.45.84l3.151-1.686v1.966l-6.252 3.35H7.551l3.437-1.841c.54-.289.09-1.13-.45-.84l-5.004 2.682H1.88l15.589-8.353v1.956zM8.382 8.309h3.6l3.913 3.913-2.344 1.256-5.169-5.169zm-3.183 3.65L.953 14.202v-1.909l2.996-1.583 1.249 1.249zM.953 15.28l4.951-2.616L7.14 13.9.953 17.169V15.28zm6.892-.675l1.257 1.257-8.149 4.367v-1.982l6.892-3.642zm9.624-3.226l-.696.373-3.442-3.442h4.139v3.069zm-4.796 2.57L9.98 15.391 2.897 8.309h4.137l5.639 5.639zM.953 8.309h.596l1.695 1.695-2.291 1.211V8.309zm12.282 12.504l4.235-2.269c.011.885.377 1.687.961 2.269h-5.196zm7.499 0c-1.275 0-2.312-1.037-2.312-2.312s1.037-2.312 2.312-2.312 2.312 1.037 2.312 2.312-1.037 2.312-2.312 2.312zm0-5.578c-.902 0-1.721.368-2.312.962v-1.876a2.34 2.34 0 0 1 2.312-2.312 2.34 2.34 0 0 1 2.312 2.312v1.876c-.592-.594-1.41-.962-2.312-.962zm2.257-3.271c-1.267-1.213-3.331-1.189-4.569.054v-.35-1.739a2.34 2.34 0 0 1 2.312-2.312 2.34 2.34 0 0 1 2.312 2.312v2.09l-.056-.054zm.001-4.392c-1.267-1.214-3.331-1.191-4.57.053V5.516c0-1.275 1.037-2.312 2.312-2.312s2.312 1.037 2.312 2.312v2.108l-.055-.053c.018.017-.20-.019 0 0zm-2.259 9.373a1.56 1.56 0 0 0-1.555 1.555 1.56 1.56 0 0 0 1.555 1.556 1.56 1.56 0 0 0 1.556-1.556 1.56 1.56 0 0 0-1.556-1.555zm0 2.158c-.332 0-.602-.27-.602-.602s.27-.602.602-.602.602.27.602.602-.27.602-.602.602zm-8.315-1.958c-.259 0-.477.218-.477.477s.218.476.477.477a.48.48 0 0 0 .476-.456.48.48 0 0 0-.476-.497z"></path></svg>
                <span class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-body-secondary rounded-circle" style="--fn-transform-scale: 1.1"></span>
              </div>
              <h3 class="h6 fw-normal mb-0">
                <a class="hover-effect-underline stretched-link" href="listings-contractors.html">Revêtements de sol</a>
              </h3>
            </article>
          </div>

          <!-- Catégorie -->
          <div class="col">
            <article class="hover-effect-scale position-relative d-inline-flex align-items-center gap-3">
              <div class="position-relative d-flex align-items-center justify-content-center text-secondary-emphasis" style="width: 56px; height: 56px">
                <svg class="position-relative z-1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"><path d="M17.183 11.986a4.74 4.74 0 0 1 .735.057.54.54 0 1 0 .168-1.071 5.85 5.85 0 0 0-.902-.07.542.542 0 1 0 0 1.084zm2.621.797c.989.664 1.686 1.664 1.963 2.814a.54.54 0 0 0 1.054-.254c-.341-1.415-1.198-2.644-2.413-3.46a.542.542 0 1 0-.604.9zm-8.356 2.713a.54.54 0 1 0 .938-.542l-.908-1.572a.542.542 0 1 0-.939.542l.908 1.572zm11.891 7.374h-.483a3.62 3.62 0 0 0 1.046-2.547c0-1.818-1.343-3.329-3.089-3.591v-.031c0-2.002-1.629-3.631-3.631-3.631a3.6 3.6 0 0 0-2.072.648l-.075-.137-2.168-3.755a.54.54 0 0 0-.74-.198l-.822.474-.291-.505a.54.54 0 0 0-.74-.198l-.821.474-3.618-6.267A2.17 2.17 0 0 0 6.193.975L5.787.271a.54.54 0 0 0-.74-.198L2.23 1.699a.54.54 0 0 0-.198.74l.406.704a2.17 2.17 0 0 0 1.881 1.084 2.14 2.14 0 0 0 .576-.079l3.619 6.268-.821.474a.54.54 0 0 0-.198.74l.291.505-.821.474a.54.54 0 0 0-.198.74l1.321 2.287c-.914.55-1.527 1.552-1.527 2.695l.002.103a2.77 2.77 0 0 0-2.549 2.756 2.75 2.75 0 0 0 .571 1.68h-.788a.542.542 0 1 0 0 1.084h19.544a.542.542 0 1 0 0-1.084zM4.858 2.998c-.251.145-.543.183-.823.108s-.513-.254-.658-.505l-.136-.235 1.877-1.084.136.235c.299.518.121 1.182-.397 1.481zm3.118 10.279l.821-.474a.54.54 0 0 0 .198-.74l-.291-.505 1.643-.949.291.505a.54.54 0 0 0 .74.198l.821-.474 1.897 3.286c.376.651.416 1.405.176 2.06l-.532-.029c-1.232 0-2.351.447-3.175 1.17a2.42 2.42 0 0 1-.693-.763l-1.897-3.286zm12.296 9.594H6.776c-.926 0-1.68-.754-1.68-1.68s.754-1.68 1.68-1.68c.106 0 .212.01.313.029a.54.54 0 0 0 .624-.674 2.04 2.04 0 0 1-.07-.534 2.06 2.06 0 0 1 .985-1.756l.305.529a3.5 3.5 0 0 0 .909 1.035 4.08 4.08 0 0 0-.24.402c-1.004.369-1.716 1.271-1.716 2.324a.542.542 0 1 0 1.084 0c0-.782.735-1.418 1.638-1.418s1.638.636 1.638 1.418a.542.542 0 1 0 1.084 0c0-1.251-1.004-2.291-2.311-2.474.641-.703 1.623-1.153 2.721-1.153 1.928 0 3.496 1.386 3.496 3.089a.542.542 0 1 0 1.084 0c0-1.789-1.242-3.319-2.982-3.911.164-.512.210-1.059.129-1.599a2.53 2.53 0 0 1 1.715-.664 2.55 2.55 0 0 1 2.547 2.547 2.57 2.57 0 0 1-.038.44.54.54 0 0 0 .554.635h.026a2.55 2.55 0 0 1 2.547 2.547 2.55 2.55 0 0 1-2.547 2.547zm-18.73.045H.639a.542.542 0 1 0 0 1.084h.903a.542.542 0 1 0 0-1.084z"></path></svg>
                <span class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-body-secondary rounded-circle" style="--fn-transform-scale: 1.1"></span>
              </div>
              <h3 class="h6 fw-normal mb-0">
                <a class="hover-effect-underline stretched-link" href="listings-contractors.html">Béton et ciment</a>
              </h3>
            </article>
          </div>

          <!-- Catégorie -->
          <div class="col">
            <article class="hover-effect-scale position-relative d-inline-flex align-items-center gap-3">
              <div class="position-relative d-flex align-items-center justify-content-center text-secondary-emphasis" style="width: 56px; height: 56px">
                <svg class="position-relative z-1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"><path d="M20.858 18.029v-7.935-1.556a.91.91 0 0 0-.626-.862v-.459A7.23 7.23 0 0 0 13.014 0a7.23 7.23 0 0 0-7.217 7.217v6.373c0 .283.23.513.513.513s.513-.23.513-.513V7.217a6.2 6.2 0 0 1 6.191-6.191 6.2 6.2 0 0 1 6.191 6.191v.414h-.978v-.414a5.22 5.22 0 0 0-5.213-5.213 5.22 5.22 0 0 0-5.213 5.213v11.039h-.978v-1.41c0-.283-.23-.513-.513-.513s-.513.23-.513.513v1.655a1.84 1.84 0 0 0-.925 1.595v.929H4.4c-.657 0-1.276.256-1.74.721a2.45 2.45 0 0 0-.721 1.741c0 .283.23.513.513.513h9.701c.283 0 .513-.23.513-.513l-.001-.033a2.61 2.61 0 0 0 1.599.546h3.968a2.61 2.61 0 0 0 .206-.009l.236.009a3.39 3.39 0 0 0 3.385-3.385c0-.998-.445-1.946-1.202-2.586zm-3.429-9.372h.286 2.004.113v.925h-2.403v-.925zM5.898 20.096c0-.449.366-.814.815-.814h1.179c.449 0 .814.365.814.814v.929H5.898v-.929zm-2.84 2.878a1.42 1.42 0 0 1 .327-.502c.271-.271.632-.421 1.015-.421h.985H9.22h.985a1.44 1.44 0 0 1 1.341.923H3.059zm8.603-7.753h-.501c-.283 0-.513.23-.513.513s.23.513.513.513h.479v.65h-.479c-.283 0-.513.23-.513.513s.23.513.513.513h.479v3.453l.003.116c-.405-.293-.902-.466-1.439-.466h-.472v-.929a1.84 1.84 0 0 0-.906-1.584V7.217c0-2.309 1.878-4.188 4.187-4.188S17.2 4.908 17.2 7.217v.421a.91.91 0 0 0-.799.9V9.74c-2.564.594-4.518 2.796-4.741 5.482zm4.528 7.693l.058.06h-1.984a1.6 1.6 0 0 1-1.598-1.598v-5.642c0-2.827 2.3-5.127 5.127-5.127h2.038v6.826c-.367-.134-.757-.203-1.157-.203a3.39 3.39 0 0 0-3.385 3.385c0 .855.320 1.672.901 2.299zm2.485.06c-.068 0-.133-.003-.196-.008a2.37 2.37 0 0 1-2.163-2.351 2.36 2.36 0 0 1 2.36-2.359 2.34 2.34 0 0 1 1.371.44c.619.443.988 1.16.988 1.919a2.36 2.36 0 0 1-2.359 2.359zm1.045-2.871c-.283 0-.513.23-.513.513a.531.531 0 1 1-1.062 0 .53.53 0 0 1 .531-.531c.283 0 .513-.23.513-.513s-.23-.513-.513-.513a1.56 1.56 0 0 0-1.557 1.557 1.56 1.56 0 0 0 1.557 1.557 1.56 1.56 0 0 0 1.556-1.557c0-.283-.23-.513-.513-.513zM6.311 14.65c-.283 0-.513.23-.513.513v.154c0 .283.23.513.513.513s.513-.23.513-.513v-.154c0-.283-.23-.513-.513-.513z"></path></svg>
                <span class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-body-secondary rounded-circle" style="--fn-transform-scale: 1.1"></span>
              </div>
              <h3 class="h6 fw-normal mb-0">
                <a class="hover-effect-underline stretched-link" href="listings-contractors.html">Nettoyage</a>
              </h3>
            </article>
          </div>

          <!-- Catégorie -->
          <div class="col">
            <article class="hover-effect-scale position-relative d-inline-flex align-items-center gap-3">
              <div class="position-relative d-flex align-items-center justify-content-center text-secondary-emphasis" style="width: 56px; height: 56px">
                <svg class="position-relative z-1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"><path d="M11.702 8.177h.677a.47.47 0 0 0 .469-.469.47.47 0 0 0-.469-.469h-.262l.239-.665a.47.47 0 0 0-.282-.6.47.47 0 0 0-.6.282l-.38 1.055a.65.65 0 0 0 .079.59c.12.172.318.274.528.274zM1.646 24h20.707a.47.47 0 0 0 .469-.469v-2.18a.47.47 0 0 0-.469-.469h-.716v-.575c0-.398-.168-.758-.436-1.013v-1.922a5.62 5.62 0 0 0-.032-.593.47.47 0 0 0-.516-.417.47.47 0 0 0-.417.516 4.71 4.71 0 0 1 .027.494v1.536h-.026-1.384a1.4 1.4 0 0 0-.214.016v-1.521a.47.47 0 0 0-.469-.469.47.47 0 0 0-.469.469v2.11a1.39 1.39 0 0 0-.247.792v.574h-4.987V19.49c.555-.194.954-.723.954-1.343v-.676c0-.785-.638-1.423-1.423-1.423s-1.423.638-1.423 1.423v.676c0 .62.399 1.149.954 1.343v1.389H6.514v-.574a1.39 1.39 0 0 0-.25-.796v-2.107a.47.47 0 0 0-.469-.469.47.47 0 0 0-.469.469v1.52a1.35 1.35 0 0 0-.212-.016H3.738V17.37c0-1.828 1.022-2.99 3.312-3.766l.983-.307 1.335 2.721c.134.273.502.346.731.145L12 14.492l1.863 1.638c.229.201.599.126.731-.148l1.309-2.701 1.049.327c.435.149.809.303 1.143.472a.47.47 0 0 0 .63-.207.47.47 0 0 0-.207-.63 9.83 9.83 0 0 0-1.27-.524l-1.451-.453-.027-.008-1.295-.404-.221-1.401a4.39 4.39 0 0 0 2.093-3.739V5.491h.046a.47.47 0 0 0 .469-.469V4.83A4.84 4.84 0 0 0 12.033 0h-.192a4.84 4.84 0 0 0-4.83 4.83v.192a.47.47 0 0 0 .469.469h.093v1.222a4.39 4.39 0 0 0 2.147 3.771l-.2 1.37-2.753.86c-1.228.415-2.13.932-2.77 1.583-.793.807-1.195 1.842-1.195 3.076v1.891c-.287.256-.469.629-.469 1.043v.575h-.686a.47.47 0 0 0-.469.469v2.18a.47.47 0 0 0 .469.469zm9.868-6.527c0-.268.218-.486.486-.486s.486.218.486.486v.676c0 .268-.218.486-.486.486s-.486-.218-.486-.486v-.676zM9.94 15.057l-1-2.037.902-.284 1.43 1.148-1.332 1.173zm4.079-.039l-1.29-1.134 1.395-1.121.033-.026.841.262-.979 2.02zM11.841.938h.192a3.9 3.9 0 0 1 3.883 3.616h-3.041a2.05 2.05 0 0 1-1.106-.329c-.347-.225-.567-.552-.797-.886-.062-.09-.118-.19-.191-.272-.201-.228-.576-.201-.743.053l-.339.516c-.241.365-.593.647-1.004.796-.237.086-.485.122-.737.122A3.9 3.9 0 0 1 11.841.938zM8.509 6.713V5.448a2.98 2.98 0 0 0 1.929-1.231 2.99 2.99 0 0 0 2.437 1.275h2.536v1.222c0 1.842-1.45 3.351-3.269 3.446a3.45 3.45 0 0 1-.535-.013C9.87 9.969 8.509 8.498 8.509 6.714zm2.682 4.321c.725.128 1.483.074 2.181-.164l.181 1.15-1.552 1.246-1.558-1.25.165-1.126a4.36 4.36 0 0 0 .585.145zm-7.922 9.274c0-.251.21-.461.461-.461h1.385a.46.46 0 0 1 .461.461v.574H3.269v-.574zm17.025-.457c.228.027.406.228.406.457v.574h-2.307v-.574a.46.46 0 0 1 .461-.461h1.385a.51.51 0 0 1 .055.004zM2.115 21.82h19.769v1.242H2.115V21.82zm17.388-6.717c.162.39.74.372.876-.028s-.318-.771-.684-.553c-.197.117-.28.37-.192.581z"></path></svg>
                <span class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-body-secondary rounded-circle" style="--fn-transform-scale: 1.1"></span>
              </div>
              <h3 class="h6 fw-normal mb-0">
                <a class="hover-effect-underline stretched-link" href="listings-contractors.html">Bricolage général</a>
              </h3>
            </article>
          </div>

          <!-- Catégorie -->
          <div class="col">
            <article class="hover-effect-scale position-relative d-inline-flex align-items-center gap-3">
              <div class="position-relative d-flex align-items-center justify-content-center text-secondary-emphasis" style="width: 56px; height: 56px">
                <svg class="position-relative z-1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"><path d="M21.337 12.788l-.791-2.789a.47.47 0 0 0-.451-.341l-2.398-.002-.138-.698a3.13 3.13 0 0 0 .634-.239c.769-.394 1.192-1.077 1.192-1.923 0-.674-.167-1.18-.367-1.682-.026-.615 1.157-1.022 1.173-1.028a.47.47 0 0 0 .306-.587.47.47 0 0 0-.588-.307c-.4.126-.936.386-1.325.777l-.102-.362c.036-1.122 1.228-1.212 1.367-1.218a.47.47 0 0 0 .45-.485.47.47 0 0 0-.485-.451 2.93 2.93 0 0 0-1.101.29 2.26 2.26 0 0 0-.444.288l-.009-.42c.016-.172.132-.675.914-.675a.47.47 0 0 0 .469-.469.47.47 0 0 0-.469-.469c-1.404 0-1.824 1.028-1.851 1.571l-.001.023c0 1.849.402 2.834.726 3.625.349.854.766 2.111-.262 2.656a2.18 2.18 0 0 1-.494.184c-.304-.832-.758-1.586-1.435-2.169-.031-.026-.064-.048-.099-.065a4.7 4.7 0 0 0-1.556-.834 2.74 2.74 0 0 0 .166-.911 2.04 2.04 0 0 0-.053-.471c.337-.103.624-.289.848-.555.784-.93.545-2.485.516-2.659a.47.47 0 0 0-.54-.382.47.47 0 0 0-.382.54c.057.331.121 1.389-.309 1.898-.146.173-.345.272-.602.303a2.01 2.01 0 0 0-.654-.43l.189-.378a.47.47 0 0 0-.21-.629.47.47 0 0 0-.629.21l-.296.592c-.012.018-.447-.004-.445 0l-.296-.592a.47.47 0 0 0-.629-.21.47.47 0 0 0-.21.629l.189.378a2.01 2.01 0 0 0-.651.427c-.243-.036-.433-.135-.577-.302-.44-.512-.398-1.55-.344-1.908a.47.47 0 0 0-.392-.534.47.47 0 0 0-.535.392c-.027.174-.239 1.729.558 2.659.219.256.497.438.819.542a2.09 2.09 0 0 0-.054.48c0 .299.059.61.165.91-.047-.132-1.512.798-1.636.905a.47.47 0 0 0-.077.086C7.549 6.513 7.1 7.203 6.78 8.072a2.24 2.24 0 0 1-.527-.186c-1.053-.54-.631-1.817-.284-2.667.323-.791.726-1.775.726-3.625l-.001-.023C6.668 1.028 6.247 0 4.843 0a.47.47 0 0 0-.469.469.47.47 0 0 0 .469.469c.782 0 .898.502.914.675l-.009.42a2.26 2.26 0 0 0-.444-.288 2.93 2.93 0 0 0-1.101-.29 2.93 2.93 0 0 0-1.101-.29c-.259-.011-.476.193-.485.451a.47.47 0 0 0 .451.485c.004 0 .364.018.718.193a1.09 1.09 0 0 1 .647 1.023l-.094.339c-.363-.374-.852-.626-1.221-.75a.47.47 0 0 0-.592.294.47.47 0 0 0 .293.594c.011.004 1.112.398 1.079 1.029-.2.502-.367 1.008-.367 1.683 0 .846.424 1.529 1.192 1.923a3.15 3.15 0 0 0 .685.252l-.14.685-2.463.002a.47.47 0 0 0-.451.341l-.791 2.789c-.068.24.074.508.323.579a.47.47 0 0 0 .579-.323l.695-2.449 1.984-.002a18.96 18.96 0 0 0-.081 1.816c0 .496.035.985.104 1.462a3.13 3.13 0 0 0-.622.236c-.769.394-1.192 1.077-1.192 1.923 0 .837.64 1.794 1.381 2.902.935 1.398 1.994 2.983 1.994 4.598a.47.47 0 0 0 .469.469.47.47 0 0 0 .469-.469c0-1.9-1.193-3.684-2.152-5.119-.629-.941-1.223-1.829-1.223-2.381 0-.646.47-1.064 1.055-1.235.275 1.128.746 2.163 1.389 3.031 1.109 1.494 2.596 2.317 4.188 2.317s3.079-.823 4.188-2.317c.644-.867 1.114-1.903 1.389-3.031.121.035.259.086.394.157.445.236.661.588.661 1.078 0 .552-.594 1.441-1.223 2.381-.959 1.434-2.152 3.219-2.152 5.119a.47.47 0 0 0 .469.469.47.47 0 0 0 .469-.469c0-1.615 1.059-3.2 1.994-4.598.741-1.108 1.381-2.066 1.381-2.902 0-.846-.424-1.529-1.192-1.923-.221-.113-.438-.187-.622-.236a10.29 10.29 0 0 0 .104-1.462l-.071-1.816 1.927.001.694 2.448a.47.47 0 0 0 .451.341c.306 0 .535-.303.451-.597zm-9.312-9.741c.525 0 1.406.134 1.406 1.031a1.87 1.87 0 0 1-.148.695 8.09 8.09 0 0 0-1.277-.097 7.86 7.86 0 0 0-1.24.095c-.094-.229-.147-.469-.147-.692 0-.897.881-1.031 1.406-1.031zm-.469 16.129c-2.495-.327-4.453-3.237-4.453-6.766 0-2.697.466-4.473 1.459-5.538l2.994 2.281v10.024zm.461-10.866L9.31 6.249c.707-.427 1.595-.636 2.695-.636 1.126 0 2.029.209 2.74.638l-2.729 2.06zm.477 10.866V9.14l3.023-2.281c.987 1.05 1.43 2.788 1.43 5.552 0 3.53-1.958 6.439-4.453 6.766zm-2.203-2.734c-.823-.93-1.315-2.437-1.315-4.032 0-1.129.096-2.024.293-2.734a.47.47 0 0 0-.327-.577.47.47 0 0 0-.577.327c-.22.793-.327 1.769-.327 2.985 0 1.845.565 3.541 1.55 4.654.165.186.468.211.662.04a.47.47 0 0 0 .04-.662zm5.397-7.038c-.065-.239-.337-.392-.576-.329a.47.47 0 0 0-.329.576c.192.702.286 1.604.284 2.759 0 .914-.159 1.807-.46 2.583a.48.48 0 0 0 .267.607c.238.092.515-.029.607-.267.357-.927.524-1.931.524-2.922-.001-1.257-.102-2.212-.318-3.006zm-1.602 7.846a.47.47 0 0 0 .469-.469.47.47 0 0 0-.469-.469.47.47 0 0 0-.469.469.47.47 0 0 0 .469.469z"></path></svg>
                <span class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-body-secondary rounded-circle" style="--fn-transform-scale: 1.1"></span>
              </div>
              <h3 class="h6 fw-normal mb-0">
                <a class="hover-effect-underline stretched-link" href="listings-contractors.html">Peinture</a>
              </h3>
            </article>
          </div>

          <!-- Catégorie -->
          <div class="col">
            <article class="hover-effect-scale position-relative d-inline-flex align-items-center gap-3">
              <div class="position-relative d-flex align-items-center justify-content-center text-secondary-emphasis" style="width: 56px; height: 56px">
                <svg class="position-relative z-1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"><path d="M15.853 5.586h-.815v-.75a.5.5 0 0 0-.5-.5h-.75V2.985h.957a.5.5 0 1 0 0-1.001h-.957v-1.4a.5.5 0 0 0-.5-.5h-2.751a.5.5 0 0 0-.5.5v1.401H8.952L7.165.151A.5.5 0 0 0 6.806 0H4.072a.5.5 0 1 0 0 1.001h2.524l1.787 1.833a.5.5 0 0 0 .358.151h1.294v1.351h-.75a.5.5 0 0 0-.5.5v.75h-.636a6.64 6.64 0 0 0-6.628 6.628V23.5a.5.5 0 0 0 .5.5h3.502a.5.5 0 0 0 .5-.5V12.214a2.13 2.13 0 0 1 2.126-2.126h1.136 5.253 1.315a2.13 2.13 0 0 1 2.126 2.126v1.376a.5.5 0 0 0 .5.5h3.502a.5.5 0 0 0 .5-.5v-1.376a6.64 6.64 0 0 0-6.628-6.628zM8.784 9.088h-.636a3.13 3.13 0 0 0-3.127 3.127V23h-2.5V12.214c0-3.103 2.525-5.628 5.628-5.628h.636v2.501h0zm2.251-8.004h1.751v.9h-1.751v-.9zm0 1.901h1.751v1.351h-1.751V2.985zm3.001 6.103H9.785V5.336h.75 2.751.75v3.752zm7.444 4.002h-2.501v-.875a3.13 3.13 0 0 0-3.127-3.127h-.815V6.587h.815c3.103 0 5.628 2.525 5.628 5.628v.875h0zm-1.327 3.835a.5.5 0 0 0-.394.192c-.268.344-1.607 2.107-1.607 3.059 0 1.103.898 2.001 2.001 2.001s2.001-.898 2.001-2.001c0-.953-1.338-2.715-1.607-3.059a.5.5 0 0 0-.394-.192zm0 4.252a1 1 0 0 1-1.001-1c0-.326.468-1.152 1.001-1.91.532.758 1 1.584 1 1.91a1 1 0 0 1-1 1zm-16.38-2.884a.5.5 0 0 0-.5.5v3.055a.5.5 0 1 0 1.001 0v-3.055a.5.5 0 0 0-.5-.5zm0-2.907a.5.5 0 0 0-.5.5v.834a.5.5 0 1 0 1.001 0v-.834a.5.5 0 0 0-.5-.5z"></path></svg>
                <span class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-body-secondary rounded-circle" style="--fn-transform-scale: 1.1"></span>
              </div>
              <h3 class="h6 fw-normal mb-0">
                <a class="hover-effect-underline stretched-link" href="listings-contractors.html">Plomberie</a>
              </h3>
            </article>
          </div>

          <!-- Catégorie -->
          <div class="col">
            <article class="hover-effect-scale position-relative d-inline-flex align-items-center gap-3">
              <div class="position-relative d-flex align-items-center justify-content-center text-secondary-emphasis" style="width: 56px; height: 56px">
                <svg class="position-relative z-1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"><path d="M15.853 5.586h-.815v-.75a.5.5 0 0 0-.5-.5h-.75V2.985h.957a.5.5 0 1 0 0-1.001h-.957v-1.4a.5.5 0 0 0-.5-.5h-2.751a.5.5 0 0 0-.5.5v1.401H8.952L7.165.151A.5.5 0 0 0 6.806 0H4.072a.5.5 0 1 0 0 1.001h2.524l1.787 1.833a.5.5 0 0 0 .358.151h1.294v1.351h-.75a.5.5 0 0 0-.5.5v.75h-.636a6.64 6.64 0 0 0-6.628 6.628V23.5a.5.5 0 0 0 .5.5h3.502a.5.5 0 0 0 .5-.5V12.214a2.13 2.13 0 0 1 2.126-2.126h1.136 5.253 1.315a2.13 2.13 0 0 1 2.126 2.126v1.376a.5.5 0 0 0 .5.5h3.502a.5.5 0 0 0 .5-.5v-1.376a6.64 6.64 0 0 0-6.628-6.628zM8.784 9.088h-.636a3.13 3.13 0 0 0-3.127 3.127V23h-2.5V12.214c0-3.103 2.525-5.628 5.628-5.628h.636v2.501h0zm2.251-8.004h1.751v.9h-1.751v-.9zm0 1.901h1.751v1.351h-1.751V2.985zm3.001 6.103H9.785V5.336h.75 2.751.75v3.752zm7.444 4.002h-2.501v-.875a3.13 3.13 0 0 0-3.127-3.127h-.815V6.587h.815c3.103 0 5.628 2.525 5.628 5.628v.875h0zm-1.327 3.835a.5.5 0 0 0-.394.192c-.268.344-1.607 2.107-1.607 3.059 0 1.103.898 2.001 2.001 2.001s2.001-.898 2.001-2.001c0-.953-1.338-2.715-1.607-3.059a.5.5 0 0 0-.394-.192zm0 4.252a1 1 0 0 1-1.001-1c0-.326.468-1.152 1.001-1.91.532.758 1 1.584 1 1.91a1 1 0 0 1-1 1zm-16.38-2.884a.5.5 0 0 0-.5.5v3.055a.5.5 0 1 0 1.001 0v-3.055a.5.5 0 0 0-.5-.5zm0-2.907a.5.5 0 0 0-.5.5v.834a.5.5 0 1 0 1.001 0v-.834a.5.5 0 0 0-.5-.5z"></path></svg>
                <span class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-body-secondary rounded-circle" style="--fn-transform-scale: 1.1"></span>
              </div>
              <h3 class="h6 fw-normal mb-0">
                <a class="hover-effect-underline stretched-link" href="listings-contractors.html">Dératisation</a>
              </h3>
            </article>
          </div>

          <!-- Catégorie -->
          <div class="col">
            <article class="hover-effect-scale position-relative d-inline-flex align-items-center gap-3">
              <div class="position-relative d-flex align-items-center justify-content-center text-secondary-emphasis" style="width: 56px; height: 56px">
                <svg class="position-relative z-1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"><path d="M19.331 17.263a4.68 4.68 0 0 0 4.671-4.672c0-1.296-.528-2.511-1.461-3.392a5.59 5.59 0 0 0 .545-2.407c0-3.081-2.507-5.588-5.588-5.588-3.004 0-5.463 2.383-5.583 5.359a5.65 5.65 0 0 0-2.624 2.34c-.296-.057-.602-.088-.914-.088-2.647 0-4.8 2.153-4.8 4.8a4.81 4.81 0 0 0 4.31 4.775v3.454H3.566a.49.49 0 0 0 0 .98h16.342a.49.49 0 0 0 0-.98h-2.895v-4.582h2.318 0zM8.378 12.306a.49.49 0 0 0-.49.49v2.594 2.014c-1.876-.241-3.331-1.847-3.331-3.788 0-2.107 1.714-3.82 3.82-3.82s3.82 1.714 3.82 3.82c0 1.941-1.455 3.547-3.331 3.788v-1.811l1.744-1.744a.49.49 0 0 0-.693-.693l-1.051 1.051v-1.411a.49.49 0 0 0-.49-.49zm7.656 9.539h-1.821l.688-.688a.49.49 0 1 0-.693-.693l-.688.688v-.959a.49.49 0 1 0-.98 0v.959l-.688-.688a.49.49 0 1 0-.693.693l.688.688h-2.98v-3.454c1.218-.124 2.303-.706 3.08-1.57.688.293 1.418.442 2.177.442h1.909v4.582zm.49-14.825a.49.49 0 0 0-.49.49v2.242l-.57-.57a1.57 1.57 0 0 0 .171-.713 1.58 1.58 0 0 0-1.577-1.577 1.58 1.58 0 0 0-1.577 1.577 1.58 1.58 0 0 0 1.577 1.577 1.57 1.57 0 0 0 .713-.171l1.263 1.263v1.938 1.088l-1.133-1.132a.49.49 0 1 0-.693.693l1.825 1.825v.735h-1.909a4.56 4.56 0 0 1-1.584-.279 4.77 4.77 0 0 0 .637-2.388c0-1.979-1.204-3.683-2.919-4.416a4.64 4.64 0 0 1 2.311-1.842.49.49 0 0 0 .324-.471l-.003-.094c0-2.541 2.067-4.608 4.608-4.608s4.608 2.067 4.608 4.608a4.61 4.61 0 0 1-.604 2.28.49.49 0 0 0 .121.626c.889.706 1.399 1.76 1.399 2.892 0 2.036-1.656 3.692-3.692 3.692h-2.318v-.938-2.067l1.744-1.744a.49.49 0 1 0-.693-.693l-1.051 1.051v-.959V7.51a.49.49 0 0 0-.49-.49zm-1.869 1.448c0 .151-.057.289-.15.394l-.028.026-.026.028a.59.59 0 0 1-.394.149.6.6 0 0 1-.597-.597.6.6 0 0 1 .597-.597.6.6 0 0 1 .597.597zM1.898 21.845H.513a.49.49 0 0 0 0 .98h1.385a.49.49 0 1 0 0-.98zm21.589 0h-1.916a.49.49 0 0 0 0 .98h1.916a.49.49 0 0 0 0-.98zM6.973 2.908c-.474-1.048-1.518-1.733-2.69-1.733a2.95 2.95 0 0 0-2.752 1.88C.664 3.199 0 3.954 0 4.862a.49.49 0 0 0 .49.49h7.588a.49.49 0 0 0 .49-.49c0-.968-.677-1.77-1.594-1.954zM1.135 4.373a.85.85 0 0 1 .697-.362l.042.001a.49.49 0 0 0 .497-.369 1.97 1.97 0 0 1 1.913-1.487 1.97 1.97 0 0 1 1.875 1.357.49.49 0 0 0 .449.337 1.01 1.01 0 0 1 .856.524H1.135z"></path></svg>
                <span class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-body-secondary rounded-circle" style="--fn-transform-scale: 1.1"></span>
              </div>
              <h3 class="h6 fw-normal mb-0">
                <a class="hover-effect-underline stretched-link" href="listings-contractors.html">Chauffage</a>
              </h3>
            </article>
          </div>

          <!-- Catégorie -->
          <div class="col">
            <article class="hover-effect-scale position-relative d-inline-flex align-items-center gap-3">
              <div class="position-relative d-flex align-items-center justify-content-center text-secondary-emphasis" style="width: 56px; height: 56px">
                <svg class="position-relative z-1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"><path d="M16.747 5.507h-.572v-3.88A1.63 1.63 0 0 0 14.548 0a1.63 1.63 0 0 0-1.627 1.627v3.88h-1.844v-3.88A1.63 1.63 0 0 0 9.45 0a1.63 1.63 0 0 0-1.627 1.627v3.88h-.572c-.626 0-1.135.509-1.135 1.135v5.402c0 1.239.638 2.406 1.666 3.044a2.55 2.55 0 0 1 1.212 2.18v.058a2.45 2.45 0 0 0 2.42 2.448v1.125a.5.5 0 1 0 1.001 0v-1.125h.138c1.35 0 2.449-1.099 2.449-2.449v-.033a2.63 2.63 0 0 1 .56-1.623 3.26 3.26 0 0 1 .192-.217c.133-.133.28-.253.441-.353 1.055-.655 1.685-1.788 1.685-3.03V6.641c0-.626-.509-1.135-1.135-1.135zm-2.824-3.88c0-.345.281-.626.626-.626s.626.281.626.626v3.88h-1.252v-3.88h0zm-5.098 0c0-.345.281-.626.626-.626s.626.281.626.626v3.88H8.825v-3.88h0zm8.056 10.442a2.55 2.55 0 0 1-1.212 2.18c-.227.141-.434.307-.62.494h-2.781a.5.5 0 1 0 0 1.001h2.082a3.64 3.64 0 0 0-.347 1.548v.033a1.45 1.45 0 0 1-1.448 1.448h-1.11a1.45 1.45 0 0 1-1.448-1.448v-.058c0-1.242-.63-2.375-1.685-3.03-.736-.457-1.193-1.297-1.193-2.193V6.641c0-.074.06-.133.133-.133h9.495c.074 0 .133.06.133.133v5.427zm-3.193-4.405a.5.5 0 0 0-.708 0l-2.67 2.67a.5.5 0 0 0 .354.854h1.461l-1.749 1.749a.5.5 0 1 0 .708.708l2.603-2.603a.5.5 0 0 0-.354-.855h-1.461l1.815-1.815a.5.5 0 0 0 0-.708zm-1.773 14.5a.5.5 0 0 0-.501.501v.834a.5.5 0 1 0 1.001 0v-.834a.5.5 0 0 0-.501-.501z"></path></svg>
                <span class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-body-secondary rounded-circle" style="--fn-transform-scale: 1.1"></span>
              </div>
              <h3 class="h6 fw-normal mb-0">
                <a class="hover-effect-underline stretched-link" href="listings-contractors.html">Électricité</a>
              </h3>
            </article>
          </div>

          <!-- Catégorie -->
          <div class="col">
            <article class="hover-effect-scale position-relative d-inline-flex align-items-center gap-3">
              <div class="position-relative d-flex align-items-center justify-content-center text-secondary-emphasis" style="width: 56px; height: 56px">
                <svg class="position-relative z-1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"><use href="#B"></use><use href="#C"></use><use href="#B" x="15.29"></use><use href="#B" y="-5.618"></use><use href="#C" y="-5.618"></use><use href="#B" x="15.29" y="-5.618"></use><use href="#D"></use><path d="M13.689 5.902c-.294 0-.533.239-.533.533A1.16 1.16 0 0 1 12 7.591c-.294 0-.533.239-.533.533s.239.533.533.533c1.225 0 2.222-.997 2.222-2.222 0-.295-.239-.533-.533-.533zm9.778-3.662H.533c-.295 0-.533.239-.533.533v14.631a4.36 4.36 0 0 0 4.356 4.356 4.36 4.36 0 0 0 3.822-2.269 4.36 4.36 0 0 0 7.644 0A4.36 4.36 0 0 0 24 17.404V2.773c0-.294-.239-.533-.533-.533zm-22.4 1.067h6.578v3.129c0 1.813-1.475 3.289-3.289 3.289S1.067 8.249 1.067 6.436V3.307zm6.578 14.098c0 1.814-1.475 3.289-3.289 3.289s-3.289-1.475-3.289-3.289v-2.767c.799.921 1.977 1.505 3.289 1.505s2.49-.584 3.289-1.505v2.767zm0-5.618c0 1.813-1.475 3.289-3.289 3.289S1.067 13.6 1.067 11.787v-2.5c.799.921 1.977 1.505 3.289 1.505s2.49-.584 3.289-1.505v2.5zm7.644 5.618c0 1.814-1.475 3.289-3.289 3.289s-3.289-1.475-3.289-3.289v-2.767c.799.921 1.977 1.505 3.289 1.505s2.49-.584 3.289-1.505v2.767zm0-5.618c0 1.813-1.475 3.289-3.289 3.289S8.711 13.6 8.711 11.787v-2.5c.799.921 1.977 1.505 3.289 1.505s2.49-.584 3.289-1.505v2.5zM12 9.724c-1.813 0-3.289-1.475-3.289-3.289V3.307h6.578v3.129h0c0 1.813-1.475 3.289-3.289 3.289zm10.933 7.68c0 1.814-1.475 3.289-3.289 3.289s-3.289-1.475-3.289-3.289v-2.767c.799.921 1.977 1.505 3.289 1.505s2.49-.584 3.289-1.505v2.767zm0-5.618c0 1.813-1.475 3.289-3.289 3.289s-3.289-1.476-3.289-3.289v-2.5c.799.921 1.977 1.505 3.289 1.505s2.49-.584 3.289-1.505v2.5zm0-5.351c0 1.813-1.475 3.289-3.289 3.289s-3.289-1.475-3.289-3.289V3.307h6.578v3.129h0z"></path><use href="#D" x="15.29"></use><defs><path id="B" d="M6.043 16.871c-.295 0-.533.239-.533.533 0 .637-.518 1.156-1.156 1.156-.295 0-.533.239-.533.533s.239.533.533.533c1.225 0 2.222-.997 2.222-2.222 0-.294-.239-.533-.533-.533z"></path><path id="C" d="M13.689 16.871c-.294 0-.533.239-.533.533 0 .637-.518 1.156-1.156 1.156-.294 0-.533.239-.533.533s.239.533.533.533c1.225 0 2.222-.997 2.222-2.222 0-.294-.239-.533-.533-.533z"></path><path id="D" d="M4.354 8.658c1.225 0 2.222-.997 2.222-2.222 0-.295-.239-.533-.533-.533s-.533.239-.533.533c0 .637-.518 1.156-1.156 1.156-.295 0-.533.239-.533.533s.239.533.533.533z"></path></defs></svg>
                <span class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-body-secondary rounded-circle" style="--fn-transform-scale: 1.1"></span>
              </div>
              <h3 class="h6 fw-normal mb-0">
                <a class="hover-effect-underline stretched-link" href="listings-contractors.html">Toiture</a>
              </h3>
            </article>
          </div>

          <!-- Catégorie -->
          <div class="col">
            <article class="hover-effect-scale position-relative d-inline-flex align-items-center gap-3">
              <div class="position-relative d-flex align-items-center justify-content-center text-secondary-emphasis" style="width: 56px; height: 56px">
                <svg class="position-relative z-1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"><path d="M12.33 14.472a.47.47 0 0 0-.517.415l-.096.874-.874-.096a.47.47 0 0 0-.517.415l-.096.874-.873-.096a.47.47 0 0 0-.517.415l-.096.874-.874-.096a.47.47 0 0 0-.517.415l-.096.874-.874-.096a.47.47 0 0 0-.517.415l-.096.874-.874-.096a.47.47 0 0 0-.517.415l-.042.39-2.104-2.104 7.363-7.363a.47.47 0 0 0 0-.663.47.47 0 0 0-.663 0l-7.694 7.694a.47.47 0 0 0 0 .663l3.127 3.127a.47.47 0 0 0 .797-.28l.098-.896.873.096a.47.47 0 0 0 .517-.415l.096-.874.874.096a.47.47 0 0 0 .517-.415l.096-.873.874.096a.47.47 0 0 0 .517-.415l.096-.873.873.096a.47.47 0 0 0 .517-.415l.096-.874.874.096a.47.47 0 0 0 .517-.415l.147-1.339a.47.47 0 0 0-.415-.517zm5.129-.986c-1.602-1.602-3.07-3.347-4.377-5.201l2.629-2.629 3.042 3.042a.47.47 0 0 0 .331.137h1.298l-.99.990-1.116-.123a.47.47 0 0 0-.517.415l-.096.874-.873-.096a.47.47 0 0 0-.517.415.47.47 0 0 0 .415.517l1.339.147c.017.002.035.003.052.003a.47.47 0 0 0 .465-.418l.096-.873.874.096a.47.47 0 0 0 .383-.135l1.948-1.948 1.621-1.621a1.83 1.83 0 0 0 0-2.587l-2.664-2.664a1.83 1.83 0 0 0-2.587 0L15.38 4.662 12.542 7.5l-.188-.282-1.019-1.55a1.12 1.12 0 0 0-.332-.791L8.192 2.065c-.323-.323-.795-.409-1.198-.259l-.251-.251a.99.99 0 0 0-.706-.293.99.99 0 0 0-.706.293L4.299 2.586a1 1 0 0 0 0 1.413l.093.093-4.18 2.736A.47.47 0 0 0 0 7.231c.072 3.202 2.636 5.766 5.838 5.838.168 0 .316-.079.403-.212l2.622-4.006.624.938c1.487 2.234 3.202 4.328 5.1 6.225l3.677 3.677a8.68 8.68 0 0 1 1.678 2.358 1.22 1.22 0 0 0 .895.671 1.25 1.25 0 0 0 .202.017c.321 0 .633-.127.864-.357l1.363-1.362a1.21 1.21 0 0 0 .357-.863 1.21 1.21 0 0 0-.357-.863l-5.806-5.806zm3.21-10.465l1.475 1.475-1.486 1.485h-.652l-1.148-1.148 1.802-1.802zm-1.792-.531c.303-.302.77-.341 1.115-.118l-2.129 2.129a.47.47 0 0 0 0 .663l1.617 1.617a.47.47 0 0 0 .331.137h1.04a.47.47 0 0 0 .331-.137l1.613-1.613c.353.338.352.901.006 1.247L21.32 7.898h-2.042l-2.904-2.905 2.503-2.503zm-13.915.758l1.031-1.031c.024-.024.063-.024.087 0l.179.179-1.118 1.118-.179-.179c-.024-.024-.024-.063 0-.087zm1.634 7.354l-.712.014a3.49 3.49 0 0 1-3.367-2.84.47.47 0 0 0-.55-.371.47.47 0 0 0-.371.55 4.47 4.47 0 0 0 1.487 2.541c.776.659 1.766 1.035 2.789 1.058l.102-.002-.371.567A5.03 5.03 0 0 1 .949 7.466l4.12-2.697 3.23 3.23-1.704 2.603zm-.792-6.425l1.449-1.449c.076-.076.2-.076.276 0L10.34 5.54c.05.05.057.108.057.138s-.007.088-.057.138L8.891 7.265 5.804 4.177zm13.718 15.504l-.596-.653-3.677-3.677a39.83 39.83 0 0 1-4.982-6.082l-.821-1.234 1.402-1.402.723 1.099a40.76 40.76 0 0 0 5.226 6.416l4.129 4.129-1.403 1.403zm3.08.674l-1.363 1.362c-.093.093-.204.086-.247.079a.28.28 0 0 1-.208-.156 9.68 9.68 0 0 0-.694-1.203l1.497-1.497 1.014 1.014a.28.28 0 0 1 .083.2.28.28 0 0 1-.083.2zm-4.729-4.34l-1.402-1.402a.47.47 0 0 0-.663 0 .47.47 0 0 0 0 .663l1.402 1.402a.47.47 0 0 0 .331.137c.119 0 .24-.046.332-.137a.47.47 0 0 0 0-.663zm-2.485-2.486c-.088-.087-.208-.137-.331-.137a.47.47 0 0 0-.469.469.47.47 0 0 0 .469.469.47.47 0 0 0 .331-.8z"></path></svg>
                <span class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-body-secondary rounded-circle" style="--fn-transform-scale: 1.1"></span>
              </div>
              <h3 class="h6 fw-normal mb-0">
                <a class="hover-effect-underline stretched-link" href="listings-contractors.html">Menuiserie</a>
              </h3>
            </article>
          </div>

          <!-- Catégorie -->
          <div class="col">
            <article class="hover-effect-scale position-relative d-inline-flex align-items-center gap-3">
              <div class="position-relative d-flex align-items-center justify-content-center text-secondary-emphasis" style="width: 56px; height: 56px">
                <svg class="position-relative z-1" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor"><path d="M19.331 17.263a4.68 4.68 0 0 0 4.671-4.672c0-1.296-.528-2.511-1.461-3.392a5.59 5.59 0 0 0 .545-2.407c0-3.081-2.507-5.588-5.588-5.588-3.004 0-5.463 2.383-5.583 5.359a5.65 5.65 0 0 0-2.624 2.34c-.296-.057-.602-.088-.914-.088-2.647 0-4.8 2.153-4.8 4.8a4.81 4.81 0 0 0 4.31 4.775v3.454H3.566a.49.49 0 0 0 0 .98h16.342a.49.49 0 0 0 0-.98h-2.895v-4.582h2.318 0zM8.378 12.306a.49.49 0 0 0-.49.49v2.594 2.014c-1.876-.241-3.331-1.847-3.331-3.788 0-2.107 1.714-3.82 3.82-3.82s3.82 1.714 3.82 3.82c0 1.941-1.455 3.547-3.331 3.788v-1.811l1.744-1.744a.49.49 0 0 0-.693-.693l-1.051 1.051v-1.411a.49.49 0 0 0-.49-.49zm7.656 9.539h-1.821l.688-.688a.49.49 0 1 0-.693-.693l-.688.688v-.959a.49.49 0 1 0-.98 0v.959l-.688-.688a.49.49 0 1 0-.693.693l.688.688h-2.98v-3.454c1.218-.124 2.303-.706 3.08-1.57.688.293 1.418.442 2.177.442h1.909v4.582zm.49-14.825a.49.49 0 0 0-.49.49v2.242l-.57-.57a1.57 1.57 0 0 0 .171-.713 1.58 1.58 0 0 0-1.577-1.577 1.58 1.58 0 0 0-1.577 1.577 1.58 1.58 0 0 0 1.577 1.577 1.57 1.57 0 0 0 .713-.171l1.263 1.263v1.938 1.088l-1.133-1.132a.49.49 0 1 0-.693.693l1.825 1.825v.735h-1.909a4.56 4.56 0 0 1-1.584-.279 4.77 4.77 0 0 0 .637-2.388c0-1.979-1.204-3.683-2.919-4.416a4.64 4.64 0 0 1 2.311-1.842.49.49 0 0 0 .324-.471l-.003-.094c0-2.541 2.067-4.608 4.608-4.608s4.608 2.067 4.608 4.608a4.61 4.61 0 0 1-.604 2.28.49.49 0 0 0 .121.626c.889.706 1.399 1.76 1.399 2.892 0 2.036-1.656 3.692-3.692 3.692h-2.318v-.938-2.067l1.744-1.744a.49.49 0 1 0-.693-.693l-1.051 1.051v-.959V7.51a.49.49 0 0 0-.49-.49zm-1.869 1.448c0 .151-.057.289-.15.394l-.28-.026-.026.028a.59.59 0 0 1-.394.149.6.6 0 0 1-.597-.597.6.6 0 0 1 .597-.597.6.6 0 0 1 .597.597zM1.898 21.845H.513a.49.49 0 0 0 0 .98h1.385a.49.49 0 1 0 0-.98zm21.589 0h-1.916a.49.49 0 0 0 0 .98h1.916a.49.49 0 0 0 0-.98zM6.973 2.908c-.474-1.048-1.518-1.733-2.69-1.733a2.95 2.95 0 0 0-2.752 1.88C.664 3.199 0 3.954 0 4.862a.49.49 0 0 0 .49.49h7.588a.49.49 0 0 0 .49-.49c0-.968-.677-1.77-1.594-1.954zM1.135 4.373a.85.85 0 0 1 .697-.362l.042.001a.49.49 0 0 0 .497-.369 1.97 1.97 0 0 1 1.913-1.487 1.97 1.97 0 0 1 1.875 1.357.49.49 0 0 0 .449.337 1.01 1.01 0 0 1 .856.524H1.135z"></path></svg>
                <span class="hover-effect-target position-absolute top-0 start-0 w-100 h-100 bg-body-secondary rounded-circle" style="--fn-transform-scale: 1.1"></span>
              </div>
              <h3 class="h6 fw-normal mb-0">
                <a class="hover-effect-underline stretched-link" href="listings-contractors.html">Jardinage</a>
              </h3>
            </article>
          </div>
        </div>
      </section>
      <!--// LISTE DES CATEGORIES DISPONIBLES -->
 
 
      <!-- NOS DERNIES PRODUIT AJOUTER -->
      <section class="container pt-2 pt-sm-3 pt-md-4 pt-lg-5 pb-5 my-xxl-3">
        <div class="d-sm-flex justify-content-between gap-3 pb-3 mb-2 mb-sm-3">
          <h2 class="mb-sm-0">Nos derniers produits ajoutés</h2>
          <ul class="nav nav-pills flex-sm-nowrap gap-2 text-nowrap">
            <li class="nav-item me-1">
              <a class="nav-link active" aria-current="page" href="#!">Voir tous les produits</a>
            </li> 
          </ul>
        </div>

        <!-- Carrousel de produits qui devient une grille sur écrans > 1200px (breakpoint xl) -->
        <div class="swiper" data-swiper="{
          &quot;slidesPerView&quot;: 1,
          &quot;spaceBetween&quot;: 24,
          &quot;pagination&quot;: {
            &quot;el&quot;: &quot;.swiper-pagination&quot;,
            &quot;clickable&quot;: true
          },
          &quot;breakpoints&quot;: {
            &quot;550&quot;: {
              &quot;slidesPerView&quot;: 2
            },
            &quot;850&quot;: {
              &quot;slidesPerView&quot;: 3
            },
            &quot;1200&quot;: {
              &quot;slidesPerView&quot;: 4
            }
          }
        }">
          <div class="swiper-wrapper">

            <!-- Produit 1 -->
            <div class="swiper-slide h-auto">
              <article class="card h-100 hover-effect-scale">
                <div class="card-img-top position-relative overflow-hidden"> 
                  <div class="ratio hover-effect-target bg-body-tertiary" style="--fn-aspect-ratio: calc(204 / 306 * 100%)">
                    <img src="assets/img/products/new-tool-kit.jpg" alt="Kit d'outils multifonctions">
                  </div>
                </div>
                <div class="card-body pb-3">
                  <div class="d-flex align-items-center justify-content-between mb-2">
                    <div class="fs-xs text-body-secondary me-3">30/09/2024</div>
                    <div class="d-flex gap-2 position-relative z-2">
                      <button type="button" class="btn btn-icon btn-sm btn-outline-secondary animate-pulse rounded-circle" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-sm" title="Ajouter aux favoris" aria-label="Ajouter aux favoris">
                        <i class="fi-heart animate-target fs-sm"></i>
                      </button>
                      <button type="button" class="btn btn-icon btn-sm btn-outline-secondary animate-shake rounded-circle" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-sm" title="Alerte prix" aria-label="Alerte prix">
                        <i class="fi-bell animate-target fs-sm"></i>
                      </button>
                    </div>
                  </div>
                  <h3 class="h6 mb-2">
                    <a class="hover-effect-underline stretched-link me-1" href="#!">Kit Outils Multifonctions</a>
                    <span class="fs-xs fw-normal text-body-secondary">32 pièces</span>
                  </h3>
                  <p class="text-body-secondary fs-xs mb-2">Kit complet avec tournevis, clés, pinces et accessoires. Boîte de rangement incluse.</p>
                  <div class="d-flex align-items-center justify-content-between">
                    <div class="h6 text-primary mb-0">
                      <span class="text-decoration-line-through text-body-secondary fs-xs me-1">49,90€</span>
                      39,90€
                    </div>
                    <button type="button" class="btn btn-primary btn-sm">
                      <i class="fi-shopping-cart"></i>
                    </button>
                  </div>
                </div>
                <div class="card-footer bg-transparent border-0 pt-0 pb-4">
                  <div class="border-top pt-3">
                    <div class="row row-cols-2 g-2 fs-xs">
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-tag"></i>
                        Outillage
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-star"></i>
                        4.7/5
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-box"></i>
                        En stock
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-truck"></i>
                        Livraison gratuite
                      </div>
                    </div>
                  </div>
                </div>
              </article>
            </div>

            <!-- Produit 2 -->
            <div class="swiper-slide h-auto">
              <article class="card h-100 hover-effect-scale">
                <div class="card-img-top position-relative overflow-hidden"> 
                  <div class="ratio hover-effect-target bg-body-tertiary" style="--fn-aspect-ratio: calc(204 / 306 * 100%)">
                    <img src="assets/img/products/eco-paint.jpg" alt="Peinture écologique">
                  </div>
                </div>
                <div class="card-body pb-3">
                  <div class="d-flex align-items-center justify-content-between mb-2">
                    <div class="fs-xs text-body-secondary me-3">15/07/2024</div>
                    <div class="d-flex gap-2 position-relative z-2">
                      <button type="button" class="btn btn-icon btn-sm btn-outline-secondary animate-pulse rounded-circle" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-sm" title="Ajouter aux favoris" aria-label="Ajouter aux favoris">
                        <i class="fi-heart animate-target fs-sm"></i>
                      </button>
                      <button type="button" class="btn btn-icon btn-sm btn-outline-secondary animate-shake rounded-circle" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-sm" title="Alerte prix" aria-label="Alerte prix">
                        <i class="fi-bell animate-target fs-sm"></i>
                      </button>
                    </div>
                  </div>
                  <h3 class="h6 mb-2">
                    <a class="hover-effect-underline stretched-link me-1" href="#!">Peinture Écologique 5L</a>
                    <span class="fs-xs fw-normal text-body-secondary">Sans COV</span>
                  </h3>
                  <p class="text-body-secondary fs-xs mb-2">Peinture écologique à base d'eau, sans odeur, pour intérieur.</p>
                  <div class="d-flex align-items-center justify-content-between">
                    <div class="h6 text-primary mb-0">54,90€</div>
                    <button type="button" class="btn btn-primary btn-sm">
                      <i class="fi-shopping-cart"></i>
                    </button>
                  </div>
                </div>
                <div class="card-footer bg-transparent border-0 pt-0 pb-4">
                  <div class="border-top pt-3">
                    <div class="row row-cols-2 g-2 fs-xs">
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-tag"></i>
                        Peinture
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-star"></i>
                        4.9/5
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-box"></i>
                        En stock
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-award"></i>
                        Certifié vert
                      </div>
                    </div>
                  </div>
                </div>
              </article>
            </div>

            <!-- Produit 3 -->
            <div class="swiper-slide h-auto">
              <article class="card h-100 hover-effect-scale">
                <div class="card-img-top position-relative overflow-hidden"> 
                  <div class="ratio hover-effect-target bg-body-tertiary" style="--fn-aspect-ratio: calc(204 / 306 * 100%)">
                    <img src="assets/img/products/smart-tool.jpg" alt="Scie circulaire intelligente">
                  </div>
                </div>
                <div class="card-body pb-3">
                  <div class="d-flex align-items-center justify-content-between mb-2">
                    <div class="fs-xs text-body-secondary me-3">16/08/2024</div>
                    <div class="d-flex gap-2 position-relative z-2">
                      <button type="button" class="btn btn-icon btn-sm btn-outline-secondary animate-pulse rounded-circle" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-sm" title="Ajouter aux favoris" aria-label="Ajouter aux favoris">
                        <i class="fi-heart animate-target fs-sm"></i>
                      </button>
                      <button type="button" class="btn btn-icon btn-sm btn-outline-secondary animate-shake rounded-circle" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-sm" title="Alerte prix" aria-label="Alerte prix">
                        <i class="fi-bell animate-target fs-sm"></i>
                      </button>
                    </div>
                  </div>
                  <h3 class="h6 mb-2">
                    <a class="hover-effect-underline stretched-link me-1" href="#!">Scie Circulaire 2000W</a>
                    <span class="fs-xs fw-normal text-body-secondary">Intelligente</span>
                  </h3>
                  <p class="text-body-secondary fs-xs mb-2">Scie circulaire avec guide laser et coupe à 45°. Puissance 2000W.</p>
                  <div class="d-flex align-items-center justify-content-between">
                    <div class="h6 text-primary mb-0">129,90€</div>
                    <button type="button" class="btn btn-primary btn-sm">
                      <i class="fi-shopping-cart"></i>
                    </button>
                  </div>
                </div>
                <div class="card-footer bg-transparent border-0 pt-0 pb-4">
                  <div class="border-top pt-3">
                    <div class="row row-cols-2 g-2 fs-xs">
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-tag"></i>
                        Électrique
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-star"></i>
                        4.8/5
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-box"></i>
                        Stock limité
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-zap"></i>
                        2000W
                      </div>
                    </div>
                  </div>
                </div>
              </article>
            </div>

            <!-- Produit 4 -->
            <div class="swiper-slide h-auto">
              <article class="card h-100 hover-effect-scale">
                <div class="card-img-top position-relative overflow-hidden"> 
                  <div class="ratio hover-effect-target bg-body-tertiary" style="--fn-aspect-ratio: calc(204 / 306 * 100%)">
                    <img src="assets/img/products/security-lock.jpg" alt="Serrure connectée">
                  </div>
                </div>
                <div class="card-body pb-3">
                  <div class="d-flex align-items-center justify-content-between mb-2">
                    <div class="fs-xs text-body-secondary me-3">19/10/2024</div>
                    <div class="d-flex gap-2 position-relative z-2">
                      <button type="button" class="btn btn-icon btn-sm btn-outline-secondary animate-pulse rounded-circle" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-sm" title="Ajouter aux favoris" aria-label="Ajouter aux favoris">
                        <i class="fi-heart animate-target fs-sm"></i>
                      </button>
                      <button type="button" class="btn btn-icon btn-sm btn-outline-secondary animate-shake rounded-circle" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-sm" title="Alerte prix" aria-label="Alerte prix">
                        <i class="fi-bell animate-target fs-sm"></i>
                      </button>
                    </div>
                  </div>
                  <h3 class="h6 mb-2">
                    <a class="hover-effect-underline stretched-link me-1" href="#!">Serrure Connectée</a>
                    <span class="fs-xs fw-normal text-body-secondary">Bluetooth</span>
                  </h3>
                  <p class="text-body-secondary fs-xs mb-2">Serrure connectée avec application mobile, empreinte digitale et code.</p>
                  <div class="d-flex align-items-center justify-content-between">
                    <div class="h6 text-primary mb-0">89,90€</div>
                    <button type="button" class="btn btn-primary btn-sm">
                      <i class="fi-shopping-cart"></i>
                    </button>
                  </div>
                </div>
                <div class="card-footer bg-transparent border-0 pt-0 pb-4">
                  <div class="border-top pt-3">
                    <div class="row row-cols-2 g-2 fs-xs">
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-tag"></i>
                        Sécurité
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-star"></i>
                        4.6/5
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-box"></i>
                        En stock
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-wifi"></i>
                        Connectée
                      </div>
                    </div>
                  </div>
                </div>
              </article>
            </div>

            <!-- Produit 5 -->
            <div class="swiper-slide h-auto">
              <article class="card h-100 hover-effect-scale">
                <div class="card-img-top position-relative overflow-hidden"> 
                  <div class="ratio hover-effect-target bg-body-tertiary" style="--fn-aspect-ratio: calc(204 / 306 * 100%)">
                    <img src="assets/img/products/watering-system.jpg" alt="Système d'arrosage">
                  </div>
                </div>
                <div class="card-body pb-3">
                  <div class="d-flex align-items-center justify-content-between mb-2">
                    <div class="fs-xs text-body-secondary me-3">22/10/2024</div>
                    <div class="d-flex gap-2 position-relative z-2">
                      <button type="button" class="btn btn-icon btn-sm btn-outline-secondary animate-pulse rounded-circle" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-sm" title="Ajouter aux favoris" aria-label="Ajouter aux favoris">
                        <i class="fi-heart animate-target fs-sm"></i>
                      </button>
                      <button type="button" class="btn btn-icon btn-sm btn-outline-secondary animate-shake rounded-circle" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-sm" title="Alerte prix" aria-label="Alerte prix">
                        <i class="fi-bell animate-target fs-sm"></i>
                      </button>
                    </div>
                  </div>
                  <h3 class="h6 mb-2">
                    <a class="hover-effect-underline stretched-link me-1" href="#!">Système d'Arrosage</a>
                    <span class="fs-xs fw-normal text-body-secondary">Automatique</span>
                  </h3>
                  <p class="text-body-secondary fs-xs mb-2">Kit d'arrosage automatique avec programmateur et 20 arroseurs.</p>
                  <div class="d-flex align-items-center justify-content-between">
                    <div class="h6 text-primary mb-0">
                      <span class="text-decoration-line-through text-body-secondary fs-xs me-1">79,90€</span>
                      64,90€
                    </div>
                    <button type="button" class="btn btn-primary btn-sm">
                      <i class="fi-shopping-cart"></i>
                    </button>
                  </div>
                </div>
                <div class="card-footer bg-transparent border-0 pt-0 pb-4">
                  <div class="border-top pt-3">
                    <div class="row row-cols-2 g-2 fs-xs">
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-tag"></i>
                        Jardinage
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-star"></i>
                        4.5/5
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-box"></i>
                        En stock
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-clock"></i>
                        Automatique
                      </div>
                    </div>
                  </div>
                </div>
              </article>
            </div>

            <!-- Produit 6 -->
            <div class="swiper-slide h-auto">
              <article class="card h-100 hover-effect-scale">
                <div class="card-img-top position-relative overflow-hidden"> 
                  <div class="ratio hover-effect-target bg-body-tertiary" style="--fn-aspect-ratio: calc(204 / 306 * 100%)">
                    <img src="assets/img/products/smart-measure.jpg" alt="Télémètre laser">
                  </div>
                </div>
                <div class="card-body pb-3">
                  <div class="d-flex align-items-center justify-content-between mb-2">
                    <div class="fs-xs text-body-secondary me-3">25/10/2024</div>
                    <div class="d-flex gap-2 position-relative z-2">
                      <button type="button" class="btn btn-icon btn-sm btn-outline-secondary animate-pulse rounded-circle" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-sm" title="Ajouter aux favoris" aria-label="Ajouter aux favoris">
                        <i class="fi-heart animate-target fs-sm"></i>
                      </button>
                      <button type="button" class="btn btn-icon btn-sm btn-outline-secondary animate-shake rounded-circle" data-bs-toggle="tooltip" data-bs-custom-class="tooltip-sm" title="Alerte prix" aria-label="Alerte prix">
                        <i class="fi-bell animate-target fs-sm"></i>
                      </button>
                    </div>
                  </div>
                  <h3 class="h6 mb-2">
                    <a class="hover-effect-underline stretched-link me-1" href="#!">Télémètre Laser 100m</a>
                    <span class="fs-xs fw-normal text-body-secondary">Professionnel</span>
                  </h3>
                  <p class="text-body-secondary fs-xs mb-2">Télémètre laser haute précision avec calcul de surface et volume.</p>
                  <div class="d-flex align-items-center justify-content-between">
                    <div class="h6 text-primary mb-0">149,90€</div>
                    <button type="button" class="btn btn-primary btn-sm">
                      <i class="fi-shopping-cart"></i>
                    </button>
                  </div>
                </div>
                <div class="card-footer bg-transparent border-0 pt-0 pb-4">
                  <div class="border-top pt-3">
                    <div class="row row-cols-2 g-2 fs-xs">
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-tag"></i>
                        Mesure
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-star"></i>
                        4.9/5
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-box"></i>
                        En stock
                      </div>
                      <div class="col d-flex align-items-center gap-1">
                        <i class="fi-target"></i>
                        Précision 1.5mm
                      </div>
                    </div>
                  </div>
                </div>
              </article>
            </div>
          </div>

          <!-- Pagination (Points) -->
          <div class="swiper-pagination position-static mt-3 mt-lg-4"></div>
        </div>
      </section>
      <!--// NOS DERNIES PRODUIT AJOUTER -->

  
    </main>

    <!-- FOOTER ...........................-->
    <?php include("./includes/footer.php") ?>
    <!-- FOOTER ...........................-->
   

</body></html>